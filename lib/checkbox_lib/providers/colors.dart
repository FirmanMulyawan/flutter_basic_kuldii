...
class MultiColor with ChangeNotifier {
  ...
  inisialData() async {
    Uri url = Uri.parse('${AppConst.appUrlFirebase}colors.json');

    try {
      var hasil = await http.get(url);
      if (hasil.statusCode >= 200 && hasil.statusCode < 300) {
        Map hasilData = (json.decode(hasil.body) ?? {}) as Map;
  ...
  void deleteColor(String id) async {
    Uri url = Uri.parse('${AppConst.appUrlFirebase}colors/$id.json');
